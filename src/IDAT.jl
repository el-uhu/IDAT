...
function plot_set(D; sp = None, xlim = [0,250], ylim = [0,1.2], drange = None, alpha = 1)
  if sp == None
    fig = figure()
    sp = subplot(111)
  end
...
function get_color(celltype, dose, treatment)
  ...
  elseif treatment == "Reversine" || treatment == "RO-3306" || treatment == "Flavopiridol"
    ...
    ranges = [t => [0:(maxima[t]/n_steps):maxima[t]] for t in keys(maxima)]
    colors = Dict(
      "HeLa" => Dict(
...
function plot_cell(x, y, c, d, t; sp = None, alpha = 1, linewidth = 2)
  if sp == None
    fig = figure()
    sp = subplot(111)
  end
...
function boxplot_set(D; sp = None, xlim = [0,250], ylim = [0,1.2], drange = None)
  if sp == None
    fig = figure()
    sp = subplot(111)
  end
